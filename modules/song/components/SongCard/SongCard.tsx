...
export const SongCard: React.FC<{ release: ReleaseProps }> = ({ release }) => {
  ...
  const isTokenOwner = React.useMemo(() => {
    ...
    return (
      ethers.utils.getAddress(signerAddress) === ethers.utils.getAddress(release?.owner)
    )
    ...
    <div>
      ...
      >
        ...
        <div className="flex w-full flex-col items-start py-2">
          <div
            className={
              'flex w-full flex-col items-start justify-between gap-1 sm:flex-row'
            }
          >
            <div className={'my-1 flex flex-col sm:my-0'}>
              <div className="text-xl font-bold">
                {release?.metadata?.artist && (
                  <Link
                    href={`${slugify(release?.metadata?.artist)}/${slugify(
                      release?.name
                    )}`}
                  >
                    {release?.name}
            ...
            {(!!auction?.highestBid || !!auction?.reservePrice) && !notForAuction && (
              <Bid release={release} />
            )}
          </div>
